...
function DashboardContent() {
  ...
  useEffect(() => {
    const currentUser = UzAuth.protectPage();
    ...
    if (roleLabel) {
      ...
      roleLabel.className = 'role-badge ' + (currentUser.role === 'tenant' ? 'tenant-banner' : 'admin-banner');
    }

    const welcomeMsg = document.getElementById('welcome-msg');
    if (welcomeMsg) {
      welcomeMsg.textContent = currentUser.fullName + ' — ' + currentUser.company;
    }

    const sel = document.getElementById('role-switcher') as HTMLSelectElement;
    const wrapper = document.getElementById('role-switcher-wrapper');
    if (sel) sel.value = currentUser.role;
    if (currentUser.role === 'tenant' && wrapper) {
    ...
    if (currentUser.role === 'tenant') {
      ...
      if (tenantContent) tenantContent.style.display = 'block';
      renderTenantView();
    } else {
      if (adminContent) adminContent.style.display = 'block';
      if (tenantContent) tenantContent.style.display = 'none';
      if (currentUser.role !== 'admin') {
        const dsTab = document.getElementById('tab-datasources');
        if (dsTab) dsTab.style.display = 'none';
      }
    }
    ...
    setTimeout(() => {
      ...
      if (cs) {
        ...
        const user = UzAuth.getCurrentUser();
        if (user?.role === 'tenant') {
          data = data.slice(0, 2);
        }
  ...
  const switchRole = (r: string) => {
    const u = UzAuth.getCurrentUser();
    if (u) {
      u.role = r as any;
      localStorage.setItem('uztenantbill_current', JSON.stringify(u));
  ...
  const renderTenantView = () => {
    ...
    const tenantData = app.tenantsData || [];
    const myTenant = tenantData[0] || { name: 'Siz', unit: 'A-12', monthly_due: 2450000, paid: 2300000, status: 'partial' };

    if (container) {
      container.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center;">
          <div>
            <div style="font-size:13px; color:#64748b;">Sizning birligingiz</div>
            <strong style="font-size:17px;">${myTenant.unit} — ${myTenant.name}</strong>
          </div>
          <div style="text-align:right;">
            <div style="font-size:13px; color:#64748b;">Hozirgi balans</div>
            <span style="font-size:22px; font-weight:800; color:#0f172a;">${(myTenant.monthly_due - myTenant.paid).toLocaleString()} UZS</span>
          </div>
        </div>
        <div style="margin-top:20px; font-size:13px;">
          <div style="display:flex; gap:20px;">
            <div><span style="color:#64748b;">To'lanishi kerak:</span> <strong>${(myTenant.monthly_due / 1000000).toFixed(1)}M</strong></div>
            <div><span style="color:#64748b;">To'langan:</span> <strong style="color:#10b981;">${(myTenant.paid / 1000000).toFixed(1)}M</strong></div>
          </div>
        </div>
      `;
    }
    if (nextDueEl) {
      nextDueEl.textContent = (myTenant.monthly_due - myTenant.paid).toLocaleString() + ' UZS';
    }
  };
    ...
    <>
      ...
      <div className="container" style={{ paddingTop: 32, paddingBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }} id="role-switcher-wrapper">
            <span style={{ fontSize: 12, color: '#64748b' }}>Demo rol:</span>
            <select id="role-switcher" onChange={e => switchRole(e.target.value)} style={{ padding: '7px 13px', border: '1px solid #e2e8f0', borderRadius: 8, fontSize: 13 }}>
        ...
        <div className="card" style={{ marginBottom: 24, padding: '16px 22px', background: 'linear-gradient(90deg,#0f172a,#1e2937)', color: '#fff' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              ...
              <button onClick={() => app.runFullPortfolioAnalysis()} className="btn btn-sm" style={{ background: '#0ea5e9', color: 'white', border: 'none' }}>Advanced Portfolio</button>
            </div>
          </div>
        </div>

        <div id="admin-content" className="role-separated">
          <div className="nav-tabs">
            ...
            <div onClick={() => switchTab('datasources')} id="tab-datasources" className="nav-tab admin-only">Ma'lumotlar</div>
            ...
            <div className="card" style={{ marginBottom: 28 }}>
              ...
              <div className="card-body" style={{ paddingTop: 6 }}>
                <div className="table-container">
                  <table>
                    <thead>
                      <tr><th>Bino</th><th>Hudud</th><th>Ijarachilar</th><th>Oylik</th><th>Yig'ish</th><th>Harakatlar</th></tr>
                    </thead>
          ...
          <div id="tab-content-proofs" style={{ display: 'none' }}>
            <div className="card">
              ...
              <div className="card-body">
                <p style={{ marginBottom: 14, fontSize: 14, color: '#64748b' }}>Har bir tugma real natija beradi. Yardi, MRI, Re-Leased, AppFolio, RealPage va Entrata bilan solishtiring.</p>
                <div className="proof-grid">
                  <button onClick={() => app.showSavingsCalculator()} className="btn btn-primary btn-sm proof-btn">💰 Savings vs Yardi</button>
                  <button onClick={() => app.compareToYardi(1)} className="btn btn-primary btn-sm proof-btn">⚖️ vs Yardi (Sergeli)</button>
                  ...
                  <button onClick={() => app.predictNextMonth()} className="btn btn-secondary btn-sm proof-btn">🔮 Next Month Forecast</button>
                  <button onClick={() => app.analyzeOccupancy()} className="btn btn-secondary btn-sm proof-btn">📈 Occupancy Uplift</button>
                  <button onClick={() => app.showPortfolioCreditScore()} className="btn btn-secondary btn-sm proof-btn">📊 Portfolio Credit</button>
                  <button onClick={() => app.forecastCashflow()} className="btn btn-secondary btn-sm proof-btn">💸 Cashflow 3mo</button>
                  <button onClick={() => app.exportToCompetitorFormat()} className="btn btn-outline btn-sm proof-btn">📥 Yardi/Re-Leased CSV</button>
                  <button onClick={() => app.simulateTenantChurnImpact()} className="btn btn-outline btn-sm proof-btn">📉 Churn Impact</button>
                  <button onClick={() => app.showMarketEdge()} className="btn btn-outline btn-sm proof-btn">🏆 Market Edge</button>
                  <button onClick={() => app.oneClick1CReconciliationLive()} className="btn btn-primary btn-sm proof-btn">1C Full Reconcile</button>
                  <button onClick={() => app.autoReconcileCAM()} className="btn btn-primary btn-sm proof-btn">Auto CAM Reconcile</button>
                  <button onClick={() => app.bulkSendTelegram()} className="btn btn-primary btn-sm proof-btn">📲 Bulk Telegram</button>
                  <button onClick={() => app.runFullPortfolioAnalysis()} className="btn btn-secondary btn-sm proof-btn">🌐 Full Portfolio</button>
                  <button onClick={() => app.showLocalPenaltyLegal()} className="btn btn-outline btn-sm proof-btn">⚖️ O'zbekiston Jarima</button>
                  <button onClick={() => app.uploadTenantProof()} className="btn btn-outline btn-sm proof-btn">📸 To'lov Tasdiqi</button>
                  <button onClick={() => app.generateGroupLink()} className="btn btn-secondary btn-sm proof-btn">📲 Telegram Guruh</button>
                  <button onClick={() => app.showInflationForecast()} className="btn btn-secondary btn-sm proof-btn">📈 CPI Bashorati</button>
                  <button onClick={() => app.bulkApplyRUBS()} className="btn btn-primary btn-sm proof-btn">🔄 Bulk RUBS</button>
                  <button onClick={() => app.exportAllFormats()} className="btn btn-primary btn-sm proof-btn">📦 Barcha Formatlar</button>
                  <button onClick={() => app.showCollectionHeatmap()} className="btn btn-secondary btn-sm proof-btn">🔥 Heatmap</button>
                  <button onClick={() => app.showFullWhyUsProof()} className="btn btn-primary btn-sm proof-btn">🏆 Nega Biz? (Live)</button>
                </div>
                <div style={{ marginTop: 18, fontSize: 13, color: '#166534', background: '#f0fdf4', padding: 10, borderRadius: 8 }}>
                  ✅ Har bir funktsiya haqiqiy UZS hisoblash, eksport yoki o'zgarish beradi. Tashkent 2026 real ma'lumotlar asosida.
                </div>
              </div>
            </div>
          </div>
        ...
        <div id="tenant-content" style={{ display: 'none' }} className="role-separated limited-view">
        ...
        <div style={{ marginTop: 26, fontSize: 12, color: '#64748b', textAlign: 'center' }}>
          2026 Toshkent real ma'lumotlari • UzTenantBill — Nega biz? Amalda isbot.
        </div>
        ...
        <div style={{ marginTop: 30, padding: 24, background: 'linear-gradient(135deg,#0a0f1c,#1e2937)', borderRadius: 20, color: '#fff', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 11.5, marginTop: 14, opacity: 0.7 }}>Live predictive analytics • Full version control • 2026 Tashkent ultra data • 120+ functions</div>
        </div>
        ...
        <div style={{ marginTop: 22 }} className="card">
          ...
          <div className="card-body">
            <div id="live-timeline" style={{ height: 102, background: '#f8fafc', borderRadius: 14, padding: 12, display: 'flex', alignItems: 'flex-end', gap: 4 }}></div>
            <div style={{ fontSize: 12, color: '#64748b', marginTop: 8, textAlign: 'center' }}>AI-generated 6-month forecast (click any bar to open detailed predictive model)</div>
          </div>
        </div>
      </div>
